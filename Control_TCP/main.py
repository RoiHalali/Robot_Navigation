...
class mywindow(QMainWindow,Ui_Client):
    def __init__(self):
        ...
        self.target_distance = 100 # For example, 100 cm
        self.current_distance = 0  # Reset when starting
        self.commandFlag = True  # allow sending
        self.start_control_loop()  # start automatic control
        self.last_update_time = time.time()  # seconds
        self.update_time_lock = threading.Lock()
        self.control_PID = True


    def start_control_loop(self):
        # Create a timer
        self.control_timer = QTimer()
        self.control_timer.timeout.connect(self.on_btn_ForWard)
        self.control_timer.start(100)  # 100 ms = 0.1 second
    ...
    #         self.lightFlag = 0
    #         self.Light.setText("Light off")
    #         ForWard=self.intervalChar+str(5000)+self.intervalChar+str(5000)+self.intervalChar+str(5000)+self.intervalChar+str(5000)+self.endChar
    #         self.TCP.sendData(cmd.CMD_MOTOR+ForWard)
    def on_btn_ForWard(self, dt_real):
        # Use dt_real for distance estimation
        error = self.target_distance - self.current_distance

        print(f"Error: {error:.2f}, Current Distance: {self.current_distance:.2f}, dt: {dt_real:.3f}")

        Kp = 0.8
        pwm_command = Kp * error

        pwm_command = min(5000, pwm_command)

        ForWard = (
                self.intervalChar + str(pwm_mapped) +
                self.intervalChar + str(pwm_mapped) +
                self.intervalChar + str(pwm_mapped) +
                self.intervalChar + str(pwm_mapped) +
                self.endChar
        ...
        rps = (pwm_command / 255) * max_rps
        speed_cm_per_sec = rps * wheel_circumference

        self.current_distance += speed_cm_per_sec * dt_real

        if abs(error) < 1:
            print("Target reached. Motors stopped.")
            self.stop_control_thread()

            StopCmd = (
                    self.intervalChar + str(0) +
                    self.intervalChar + str(0) +
                    self.intervalChar + str(0) +
                    self.intervalChar + str(0) +
                    self.endChar
            )
            self.TCP.sendData(cmd.CMD_MOTOR + StopCmd)
    ...
    def control_loop(self):
        ...
        while self.controlFlag:
            ...
            self.on_btn_ForWard(dt_real)  # Pass real dt to control function

            print(f"Loop ran, dt_real: {dt_real:.4f} sec")

            time.sleep(0.1)  # Try to sleep for 100 ms (will be adjusted by processing time)

    def start_control_thread(self):
        self.controlFlag = True
        self.controlThread = threading.Thread(target=self.control_loop)
